...
SamplePlayer = function(bp, onEnd, onUpdate, onInitialized) {
	this.title;
	this.author;
	this.desc;
	this.program;
...
SamplePlayer.prototype = {
	...
	installBIOS: function (file) {
		var reader = new FileReader();
		reader.onload = function() {
			var d= new Uint8Array(reader.result);	
			var f= Module.FS_createDataFile("/", file.name, d, true, true);	// now the C code will find it

			// if not explicitly set here.. then 'emu_init' will go for some default later..
			if (Module.ccall('emu_setup', 'number', ['string'], [file.name]) ==0) {
				this.onInitialized();
			} else {
	...
	updateTrackInfos: function(filename) {
		...
		this.program= Module.Pointer_stringify(array[3]);
		this.subPath= Module.Pointer_stringify(array[4]);
		this.tracks= Module.Pointer_stringify(array[5]);
		this.currentTrack= Module.Pointer_stringify(array[6]);
	},
